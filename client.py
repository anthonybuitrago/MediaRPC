...
class MediaRPCClient:
    def __init__(self):
        ...
        # [EXTENSION]
        self.extension_state = {} # Stores state per source: {'hianime': {...}, 'youtube_music': {...}}
    ...
    def _on_extension_update(self, data):
        ...
        self.extension_state[source] = {
            "data": data,
            "timestamp": time.time()
        }

    def _handle_extension_rpc(self):
        """Maneja datos provenientes de la extensión (Prioridad: HiAnime > YouTube Music)."""
        
        # --- 1. HiAnime Logic ---
        hianime_state = self.extension_state.get("hianime")
        
        # Check if HiAnime is active (recent data < 3s)
        if hianime_state and (time.time() - hianime_state["timestamp"] < 3):
            data = hianime_state["data"]
            
            # Check if Playing (Playing or Mixed+Visible)
            # Logic: If it's playing, we SHOW it. If it's hidden/paused, we PASS to next source.
            is_playing = data.get("is_playing", False)
            
            # Additional Check: If it says "is_hidden", we consider it paused/inactive for RPC purposes.
            if is_playing and not data.get("is_hidden"):
                 return self._process_hianime_data(data)
            else:
                 # HiAnime is present but hidden/paused -> CLEAR internal HiAnime RPC state
                 if self.last_source == "extension_hianime":
                    try: self.rpc.clear()
                    except: pass
                    self.last_source = None
                    self.hianime_last_data = None
                 # FALLTHROUGH: Continue to check Music
        
        # --- 2. YouTube Music Logic ---
        ytm_state = self.extension_state.get("youtube_music")
        
        if ytm_state and (time.time() - ytm_state["timestamp"] < 3):
            data = ytm_state["data"]
            
            if data.get("is_playing"):
                 return self._process_ytm_data(data)

        return False

    def _process_ytm_data(self, data):
        """Helper for YouTube Music RPC update"""
        ...
        # [FIX] Resume Bug: Only skip update if we are CONTINUOUSLY on music source.
        # If we came from None (cleared) or another source, we must Force Update.
        if (self.last_source == "extension_music" and
            getattr(self, "last_media_name", "") == data['title'] and
    ...
    def _process_hianime_data(self, data):
        """Helper for HiAnime RPC update"""
        target_id = self.config["client_id"] 
        self.connect_discord(target_id)
        ...
        if data.get("type") in ["meta", "mixed", "playback"] and data.get("title"):
        ...
        # Opción A: Tenemos tiempo exacto del video (Playback Mode) - RARO en iframe
        if data.get("current_time") and data.get("total_time") and data.get("type") == "playback":
            now = time.time()
            start_ts = now - data["current_time"]
        
        # Opción B: No tenemos video (Mixed/Meta Mode) -> Usamos tiempo sesión
    ...
    def _cleanup_rpc(self):
        ...
        if self.last_source:
            ...
            self.last_source = None
            self.hianime_last_data = None # [FIX] Reset cache so next update is treated as fresh
            self.last_media_name = None # [FIX] Reset music cache
    ...
    def run_logic(self):
        ...
        while self.running:
            ...
            try:
                ...
                for src_data in self.extension_state.values():
                     if time.time() - src_data["timestamp"] < 3: # [MODIFIED] 10s -> 3s
